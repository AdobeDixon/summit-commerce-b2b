#!/usr/bin/env node
/**
 * Update Adobe Commerce catalog product names (same titles as EQUIPMENT_CATALOG_NAMES).
 * SKUs are unchanged; only the `name` field is sent.
 *
 * Prerequisites
 * - Products with these SKUs must exist in the catalog.
 * - IMS credentials need permission to update products (per Adobe Commerce as a Cloud Service).
 *
 * Usage:
 *   DRY_RUN=1 node scripts/update-commerce-product-names.mjs
 *   node scripts/update-commerce-product-names.mjs
 *   npm run update-product-names
 *
 * Env (same family as scripts/create-commerce-user.mjs):
 *   API_ENDPOINT          - REST API base (optional; from config.json commerce-endpoint minus /graphql)
 *   IMS_CLIENT_ID         - Adobe IMS OAuth client id
 *   IMS_CLIENT_SECRET     - Adobe IMS client secret
 *   IMS_ORG_ID            - Optional; sent as x-gw-ims-org-id
 *   MAGENTO_REST_ROOT     - Optional path segment before V1. Default empty (SaaS):
 *                           {API_ENDPOINT}/V1/products/:sku
 *                           For on-premises Magento use e.g. rest/default so the path becomes
 *                           {origin}/rest/default/V1/products/:sku — set API_ENDPOINT to store origin only.
 *   DRY_RUN=1             - Log planned updates only; no HTTP writes.
 *
 * Alternative (self-hosted Magento admin/integration token — not Adobe IMS):
 *   COMMERCE_ACCESS_TOKEN - If set, IMS auth is skipped and this Bearer token is used.
 *   When using this, set API_ENDPOINT to your site base (e.g. https://shop.example.com) and
 *   MAGENTO_REST_ROOT=rest/default (or your store view code).
 */
...
/** Store scope for REST (matches storefront GraphQL headers in config.json). */
...
async function getImsAccessToken() {
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      'Missing IMS_CLIENT_ID / IMS_CLIENT_SECRET (or set COMMERCE_ACCESS_TOKEN for a Magento admin token).',
    );
  }
...
async function putProductName(baseUrl, auth) {
  ...
  const entries = Object.entries(EQUIPMENT_CATALOG_NAMES);
  let ok = 0;
  let failed = 0;

  for (const [sku, name] of entries) {
    const url = buildProductUrl(baseUrl, sku);

    if (dryRun) {
      console.log(`[dry-run] GET ${url} then PUT name`);
    ...
    if (response.ok) {
      ...
      console.error(`ERR ${sku} HTTP ${response.status}`, typeof data === 'string' ? data : JSON.stringify(data, null, 2));
      failed += 1;
    }
  }
...
async function main() {
  ...
  console.log('Products to update:', Object.keys(EQUIPMENT_CATALOG_NAMES).length);
  ...
  await putProductName(baseUrl, auth);
}
